...
contract Planets is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    event attackInitated(
        uint indexed attackedPlanet,
        address indexed Attacker,
        uint indexed timeToArrive,
        uint arrayIndex
    );
    ...
    function getAttack(uint _instanceId)
        external
        view
    ...
    function getDefensePlanet(uint _planetId)
        external
        view
        returns (uint[] memory)
    {
    ...
    function assignDefensePlanet(uint _planetId, uint[] memory _newDefenseShips)
        external
        onlyGameDiamond
    {
